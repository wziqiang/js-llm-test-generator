...
function wrapSourceWithExports(code) {
  ...
  // 1. 匹配传统函数定义: function name(...) { ... }
  const funcRegex = /function\s+([a-zA-Z0-9_$]+)\s*\(/g;
  // 2. 匹配箭头函数赋值: const name = (...) => { ... }
  ...
  if (functionNames.size > 0) {
    const exportList = Array.from(functionNames).join(", ");
...
//     "```",
//     "",
//     "请仅输出代码（以 ```javascript 开头），不要任何文字解释。",
//   ].join("\n");
// }
function buildPrompt(sourceCode) {
  return [
...
app.post("/api/generate-and-test", async (req, res) => {
  try {
    ...
    const testCodeClean = testCode.trim();
